...
export default class BufferReader {
	...
	readString(enc: string, size: number) {
		assert(size >= 0);
		this.assert(this.offset + size <= this.data.length);
	...
	readVarString(enc: string, limit?: number) {
